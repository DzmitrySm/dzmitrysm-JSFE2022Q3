...
audio.onloadeddata = () => {
  console.log(`${Math.floor(audio.duration / 60)}:${Math.round(audio.duration % 60)}`);
}

function progressSong() {
  let durationS = audio.onloadeddata = () => {
    let dur = `${Math.floor(audio.duration / 60)}:${Math.round(audio.duration % 60)}`;
    console.log(dur)
    let currentPoint = audio.currentTime;
...
durationRange.addEventListener('input', () => {
  audio.duration = durationRange.value;
})
